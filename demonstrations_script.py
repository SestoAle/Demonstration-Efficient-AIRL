...
# Minigrid task
    ...
    environment = MinigridEnvWrapper(game_name="MiniGrid-MultiRoom-N3-S5-v0", manual_input=False, verbose=False,
                                     no_graphics=True, sample_env=int(args.sampled_env))
...
try:
    ...
    if task_name == 'potions':
        reward_model = AIRL(actions_size=17, policy=agent, network=potions_net, input_architecture=deepcrawl_input,
                            obs_to_state=deepcrawl_obs_to_state, sess=sess, dems_name=args.demonstrations_name,
                            lr=1e-5, entropy_weight=0.1, batch_size=32, num_itr=20, eval_with_probs=True)
    elif task_name == 'minigrid':
        reward_model = AIRL(actions_size=4, policy=agent, network=multiroom_net, input_architecture=minigrid_input,
                            obs_to_state=minigrid_obs_to_state, sess=sess, dems_name=args.demonstrations_name,
                            lr=5e-6, entropy_weight=0.01, batch_size=32, num_itr=20, clipping=False, eval_with_probs=False)

    init = tf.compat.v1.global_variables_initializer()
    sess.run(init)

    if args.reward_model is not None and reward_model is not None and args.fixed_reward_model:
        reward_model.load_model(args.reward_model)
        print("Model loaded!")
