...
class UnityEnvWrapper(Environment):
    def __init__(self, game_name = None, no_graphics = True, seed = None, worker_id=0, size_global = 8, size_two = 5, with_local = True, size_three = 3, with_stats=True, size_stats = 1,
                 ...
                 with_class=False, with_hp = False, size_class = 3, use_double_agent = False, double_agent = None, reward_model = None, _max_episode_timesteps=100):
        ...
        self.unity_env = self.open_environment(game_name, no_graphics, seed, worker_id)
    ...
    def get_input_observation(self, env_info, action = None):
        ...
        if self.with_local and self.with_stats and self.with_previous:
            ...
            observation = {
                ...
                # Stats
                'agent_stats': stats[:14],
                'target_stats': stats[14:],
    ...
    def execute(self, actions):
        ...
        while env_info == None:
            ...
            try:
                ...
                self.close()
                self.unity_env = self.open_environment(self.game_name, self.no_graphics, seed = int(time.time()),
        ...
        if self.use_double_agent:
            while len(env_info.vector_observations) <= 0:
                ...
                if np.random.rand() > 0.33:
                    act, fetch = self.double_agent.act(states=obs, deterministic=False, independent=True,
                                             query=query)
                else:
    ...
    def open_environment(self, game_name, no_graphics, seed, worker_id):
        return UnityEnvironment(game_name, no_graphics=no_graphics, seed=seed, worker_id=worker_id)

    def re_open_environment(self):
        self.close()
        self.unity_env = self.open_environment(self.game_name, self.no_graphics, self.seed, self.worker_id)
